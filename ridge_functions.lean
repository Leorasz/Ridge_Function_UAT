...
theorem trivial_annihilator_implies_eq (Ω : Finset (Fin n → ℝ)) (m : ℕ)
  ...
  have heq : (polyHom K) p = ridgeFunction K Ω a ⟨fun x => x ^ m, continuous_pow m⟩ := by
    ext x
    simp [ridgeFunction, ContinuousMap.coe_mk, polyHom, p, q, coord]
  exact heq ▸ Submodule.mem_map_of_mem hp
...
lemma mono_submod_ridges (Ω : Finset (Fin n → ℝ)) (m : ℕ) : monoRidges K Ω m ≤ ridges K Ω := by
...
theorem homogeneous_ridged_implies_polynomials (Ω : Finset (Fin n → ℝ))
  ...
  have h_each_in_ridges (m : ℕ) (hm : m ∈ Finset.range (p.totalDegree + 1)) :
    ...
    rw [h m] at this
    exact mono_submod_ridges K Ω m this
...
theorem ridges_closed (Ω : Finset (Fin n → ℝ)) (h_annihilator : ∀m, (monoPoly K Ω m).dualAnnihilator = ⊥) :
  ...
  refine dense_of_contains_polySubalg K ?_
  refine homogeneous_ridged_implies_polynomials K Ω fun m => ?_
  exact trivial_annihilator_implies_eq K Ω m (h_annihilator m)
